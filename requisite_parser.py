...
class RequisiteParseTree():
    ...
    def likely_is_course(self, requisite):
        ...
        if len(requisites_split) == 1:
            ...
            return requisites_split[1][0].isdigit()
        else:
            return False
    ...
    def process(self):
        ...
        print('root preprocessed:', self.root)
        self.flatten()
        print('root flattened:', self.root)
        self.infer_subjects()
        print('root inferred:', self.root)
    ...
    def second_level_split(self, requisite_string):
        ...
        (prefix, p), (suffix, s) = self.find_prefix_and_suffix(requisite_string)
        if (isinstance(prefix, str) and isinstance(suffix, str)) and (prefix.lower() == 'registration in' and suffix.lower() == 'program'):
            ...
            if prefix is not None or suffix is not None:
                ...
                if prefix is not None and prefix.lower() in {'one of', 'either'}:
                    requisite_cleaned = requisite_cleaned
                elif prefix is not None and prefix.lower() == 'credit or registration in one of':
        ...
        for i, requisite in enumerate(requisites_split_and_or[0:-1]):
            ...
            if AND_OR_list[i] == 'or':
                ...
                requisites_split_and_or = [
                    ...
                    ' or '.join([requisites_split_and_or[j].replace(',', ' or'), requisites_split_and_or[j+1]]) \
                ...
                requisites_split_and_or = [
                    ...
                    ' and '.join([requisites_split_and_or[j].replace(',', ' and'), requisites_split_and_or[j+1]]) \
    ...
    def seventh_level_v2_split(self, requisite_string):
        ...
        elif prefix is not None and prefix.lower() in {'all of', 'either'}:
...
if __name__ == '__main__':
    ...
    """
    rpt = RequisiteParseTree(requisites)
    rpt.process()
